            ...
            RageUI.IsVisible(gofast_menu, function()
                ...
                RageUI.Button("Commencer un ~r~GoFast", nil, {}, not GoFastEnCours, {
                    onSelected = function ()
                        ...
                        ESX.ShowAdvancedNotification("GoFast", "", "Livres moi cette ~y~"..Config.Vehicle[Random].label.."~s~ au point ~y~GPS", "CHAR_MP_MEX_BOSS", 8)
                        ...
                        TriggerServerEvent("skn:CallPolice")
...
CreateThread(function()
    while true do 
        ...
        for k,v in pairs(Config.GoFastPos) do 
            ...
            if dist <= 3 then
                interval = 0
                Visual.Subtitle("Appuyer sur [~b~E~s~] pour commencer un ~r~Gofast", 1)
        ...
        for k,v in pairs(Config.GoFastPosVente) do 
            ...
            if dist <= 2.0 and GoFastEnCours then 
                ...
                Visual.Subtitle("Appuyer sur [~e~E~s~] pour commencer un ~r~Gofast", 1)
                if IsControlJustPressed(1, 51) then 
                    GoFastFini()
                    GoFastEnCours = false
                end
...
function GoFastFini()
    ...
    if plate == " GOFAST " then 
        TriggerServerEvent("skn:GoFastFini")
